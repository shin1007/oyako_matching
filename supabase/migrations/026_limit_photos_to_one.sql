...
BEGIN
  ...
  FOR photo_record IN 
    ...
    FROM public.searching_children_photos 
...
DELETE FROM public.searching_children_photos
WHERE display_order > 0;
...
WHERE id IN (
  SELECT p.id
  FROM public.searching_children_photos p
  WHERE EXISTS (
    SELECT 1
    FROM public.searching_children_photos p2
...
RETURNS TRIGGER AS $$
BEGIN
  IF (SELECT COUNT(*) FROM public.searching_children_photos WHERE searching_child_id = NEW.searching_child_id) >= 1 THEN
...
-- （022_searching_children_photos.sql で作成済み）

-- ===== コメント更新 =====

COMMENT ON COLUMN public.searching_children_photos.display_order IS '表示順序（現在は常に0、将来の拡張用に保持）';
