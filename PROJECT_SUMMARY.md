...
親子断絶・実子誘拐後の再会を支援するマッチングプラットフォームです。AIとマイナンバーカード認証による厳格な本人確認とエピソードマッチングで、安全かつ確実な親子の再会をサポートします。
...
### 3. エピソード登録
- 思い出のエピソードを文章で登録
- OpenAI APIによる自動モデレーション
- ベクトル化による類似度計算の基礎データ

### 4. AIマッチング
- **ベクトル検索**
  - エピソードをOpenAI embeddingsでベクトル化
  - PostgreSQL pgvectorで類似度検索
  - コサイン類似度スコアでランキング
- **類似度閾値**: 70%以上
- **段階的マッチング**
  - 生年月日による絞り込み
...
- **Stripe** - サブスクリプション決済
- **OpenAI API**
  - text-embedding-3-small（エピソード埋め込み）
...
1. **users** - ユーザー基本情報
2. **profiles** - プロフィール
3. **episodes** - エピソード（ベクトル含む）
4. **matches** - マッチング関係
5. **messages** - メッセージ
6. **time_capsules** - タイムカプセル
7. **subscriptions** - サブスクリプション情報
...
- `/dashboard/episodes` - エピソード管理
- `/dashboard/time-capsules` - タイムカプセル（親のみ）
