...
このドキュメントでは、探している子ども・親の写真を登録する機能について説明します。この機能により、ユーザーは探している相手の写真を1枚登録でき、撮影日時と年齢情報を記録することができます。

将来的には、これらの情報を利用してCloudflare Pagesの機能で現在の姿を推定する機能を追加する予定です。

**注意**: 2026-01-19にサーバー負荷軽減のため、写真登録数を最大5枚から1枚に変更しました。
...
| display_order | INTEGER | 表示順序（現在は常に0、将来の拡張用に保持） |
...
4. JPEG、PNG、またはWebP形式の画像を選択（最大5MB、1枚のみ）
5. アップロード後、以下の情報を入力:
   - **撮影日**: 写真を撮影した日付（任意）
   - **撮影時の年齢**: その時の年齢（任意）
...
- 1つの子ども・親につき1枚のみ写真を登録できます（2026-01-19変更）
- 画像は自動的に圧縮されます（最大5MB）
