...
## 実装上の注意点・バリデーション・エラー応答例

### パラメータバリデーション
- `page`：1以上の整数のみ許可（デフォルト1）
- `limit`：1〜100の整数のみ許可（デフォルト20、最大100）
- 不正な値は自動的に補正される
...
### エッジケース・運用上の注意
- マッチが0件の場合も `matches: []` と `pagination` オブジェクトを返す
- ページネーションの `total_pages` は常に計算される
- 取得件数が0件でもエラーにはならない
- 取得範囲外のページ指定も空配列で返却

### 今後の拡張・改善案（実装検討中）
- **カーソルベースページネーション**：大規模データ対応・パフォーマンス向上
- **キャッシュ導入**：Redis等によるAPI応答高速化
- **GraphQL対応**：必要なフィールドのみ取得し、通信量削減
- **マテリアライズドビュー**：未読数・最終メッセージの事前計算
